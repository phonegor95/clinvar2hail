...
def read_and_filter_variant_summary(file_path, chromosome_criteria, review_status_criteria, clinical_significance_pathogenic, variant_type_filter):
    header_line, skip_rows = get_header_line(file_path)
    df = pd.read_csv(file_path, sep='\t', skiprows=skip_rows, names=header_line, low_memory=False)
    ...
    df = df[(df['PositionVCF'] != -1) & 
            (df['ReferenceAlleleVCF'] != 'na') & 
            (df['AlternateAlleleVCF'] != 'na')]
    ...
    df = df[~df['Type'].isin(variant_type_filter)]
...
def read_and_filter_submission_summary(file_path, phenotype_filter):
    header_line, skip_rows = get_header_line(file_path)
    df = pd.read_csv(file_path, sep='\t', skiprows=skip_rows, names=header_line, low_memory=False)
    df = df[~df['ReportedPhenotypeInfo'].isin(phenotype_filter)]
...
def merge_and_prepare_for_hail(submission_df, variant_df):
    merged_df = submission_df.merge(variant_df, on='VariationID')
...
def main():
    ...
    variant_submission_ht = merge_and_prepare_for_hail(submission_df, variant_df)
...
if __name__ == "__main__":
    ...
    gnomad = args.gnomad
    date = args.date
    output_path = work_dir + 'clinvar_summary_' + date + '_w_gnomad.ht'
